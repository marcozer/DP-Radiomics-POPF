...
- `data_templates/`: schema descriptions and synthetic placeholders that explain how to structure radiomics and clinical CSVs without shipping PHI.
...
Populating these folders completes the export handoff: we copy the vetted scripts under `code/`, place sanitized configuration/data templates, drop the frozen STABL panel + figures into `results_reference/`, and provide the `docs/` artifacts to describe how to recreate full runs on a clean machine.
...
- For current R0_v2 manuscript analysis, use `code/models/r0_v2_elasticnet_7rad_mpd_thickness.py` with local, non-versioned radiomics and clinical CSVs.
