...
- `primary analysis/`: frozen model + frozen calibration → POPF risk.
...
- `deployment/radpanc_runner.py`: an end-to-end CLI that:
  ...
  6) computes **raw + calibrated** POPF risk using the exported model bundle.
...
- Compact side panels for upload, pipeline progress, live logs, and the final calibrated POPF risk.
...
## Calibration (final reportable risk)

The **final reportable risk** is the **calibrated** probability.

- Calibration is generated by `primary analysis/code/models/comparative_risk_stratification_v2.py`.
- A frozen calibration artifact is stored under:
  `primary analysis/configs/calibration/radiomics_calibration.json`.
- `primary analysis/code/predict_popf_risk.py` will auto-use that default calibration unless you pass `--no-calibration`.

## Risk stratification (Low / Intermediate / High)

- The deployment assigns a **frozen** risk group based on the **calibrated** probability.
