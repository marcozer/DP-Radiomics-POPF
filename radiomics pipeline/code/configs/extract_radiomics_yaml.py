...
# Local default input/output directories (override with env/CLI)
...
# Mutable globals used by the class
...
class YAMLRadiomicsExtractor:
    ...
    def process_dataset(self, input_dir=None):
        ...
        if all_features:
            ...
            output_path = OUTPUT_DIR / f"radiomics_yaml_{timestamp}.csv"
    ...
    def generate_extraction_report(self, df, feature_counts, failed_patients, timestamp):
        ...
        report_path = OUTPUT_DIR / f"extraction_report_yaml_{timestamp}.json"
    ...
    def save_methods_text(self, timestamp):
        ...
        if 'methods_text' in self.config:
            methods_path = OUTPUT_DIR / f"methods_section_{timestamp}.txt"
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default=None,
                       help='Directory to write radiomics outputs')
    parser.add_argument('--validate-only', action='store_true',
                       help='Only validate configuration without extraction')
    
    args = parser.parse_args()

    # Allow CLI override of globals used by the extractor
    ...
    extractor = YAMLRadiomicsExtractor(config_path)
